...
class HybridImageModel(nn.Module):
    ...
    def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
        ...
        k = kernel.shape[0]
        kernel = torch.Tensor(kernel).view(1, 1, k, k)
        kernel = kernel.repeat(self.n_channels, 1, 1, 1)  # Repeat for each channel
    ...
    def low_pass(self, x: torch.Tensor, kernel: torch.Tensor):
        ...
        filtered_image = F.conv2d(
            x, kernel, padding=pad, groups=self.n_channels, stride=1
        )
